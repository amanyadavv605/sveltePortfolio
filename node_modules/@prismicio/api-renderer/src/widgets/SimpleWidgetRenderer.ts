...
const SimpleWidgetRenderer = (ctx: RenderContext) =>
	withObjectRenderers<NestableWidget, NestableContent, Field>({
		renderV1(content, fetch) {
			...
			} else if (isTableContent(content)) {
				return TableRenderer(ctx).renderV1(content, extractSimpleFieldIfAny(fetch))
			} else {
				return null
			}
		},
		...
		renderV2(def, content, fetch) {
			...
			} else if (def.type === "Table" && isTableContent(content)) {
				return TableRenderer(ctx).renderV2(def, content, extractSimpleFieldIfAny(fetch))
			} else {
				return null
			}
		},
		...
		renderDefault(def) {
			if (def.type === "Link") {
				...
				return TableRenderer(ctx).renderDefault(def)
			} else {
		...
		renderMocks(def, content) {
			if (def.type === "StructuredText" && isRichTextContent(content)) {
				...
				return TableRenderer(ctx).renderMocks(def, content)
			} else {
				return null
			}
		},
