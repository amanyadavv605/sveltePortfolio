	...
	return {
		renderV1(
			...
			try {
				if (FilledDocumentLinkContent.is(link.value)) {
					...
					const id = extension.DocEncoder.encodeDocId(link.value.id, fetch?.uuid, [
						linkKey,
						link.value.text,
						link.value.variant,
					])
		...
		renderV2(
			...
			try {
				...
				if (FilledDocumentLinkContent.is(link.value)) {
					const linkKey = options?.omitKey ? undefined : link.key
					const id = extension.DocEncoder.encodeDocId(link.value.id, fetch?.uuid, [linkKey, linkText, linkVariant])
